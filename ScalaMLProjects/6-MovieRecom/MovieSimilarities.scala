...
object MovieSimilarities {
  ...
  // numRatersPerMovie.collect().foreach(println)
  ...
  //ratingsWithSize.collect().foreach(println)

  // The user U for the movie M rated that with R (1,2,3,4,5 stars) and the movie M received totalRates NR
  ...
  val ratingPairs = ratingsWithSize.keyBy(tup => tup._1)
                                   .join(ratings2)
                                   .filter(f => f._2._1._2 < f._2._2._2)

   // ratingPairs.take(20).foreach(x => println(x))
