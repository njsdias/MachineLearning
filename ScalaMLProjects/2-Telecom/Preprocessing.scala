...
//  trainSet.show()
  ...
  val churnDF = trainSet.stat.sampleBy("churn", fractions, 123456L) 
    ...
    /* By data inspection we saw that total day minutes and total_day_charge are 
   * a highly correlated feature in this training set, which is not beneficial for our ML
   * model training. Therefore, it would be better to remove them altogether  */
  //Let's drop one column of each pair of correlated fields
  val trainDF = churnDF
        .drop("state_code")               // see the distribution on correlation plot
        .drop("area_code")                // see the distribution on correlation plot
        .drop("voice_mail_plan")          // see the distribution on correlation plot
        .drop("total_day_charge")         // correlated with total_day_mins
        .drop("total_evening_charge")     // correlated with total_evening_mins
  ...
  trainDF.select("account_length", "international_plan", "num_voice_mail", "total_day_calls", "total_international_num_calls", "churn").show(10) 
   ...
   /*However, we are not done yet; the current DataFrame cannot be fed to the model as an
   * estimator. As we described, the Spark ML API needs our data to be converted in a Spark
   * DataFrame format, consisting of a label (in Double) and features (in Vector).*/

  // Now we need build a pipeline: PipelineConstructor.scala 
